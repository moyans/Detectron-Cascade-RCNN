...
def main(args):
    ...
    import numpy as np
    dets_j = np.hstack((proposal_boxes, _proposal_scores[:, np.newaxis])).astype(
        np.float32, copy=False
    )
    keep = box_utils.nms(dets_j, NMS)
    nms_dets = dets_j[keep, :]
    # finnal_bbox = nms_dets

    # Limit to max_per_image detections **over all classes**
    cls_boxes[1] = nms_dets
    ...
    finnal_bbox = np.vstack([cls_boxes[j] for j in range(1, num_classes)])
    ...
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    # boxes = nms_dets
    # if (boxes is None or boxes.shape[0] == 0 or max(boxes[:, 4]) < thresh) and not False:
    #     return
    # if boxes is None:
