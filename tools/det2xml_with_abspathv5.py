...
output_sub_img = '/project/data/label_sys_sdi/DATA/split_{}'.format(
...
def walkDir2RealPathList(path, filter_postfix=[".jpg", ".JPG", "PNG", ".png", ".jpeg"]):
    ...
    if filter_postfix:
        print("Files will be searched by the specified suffix, {}".format(
            filter_postfix))
    else:
    ...
    def detect(self, img):
        ...
        for i in sorted_inds:
            ...
            if score < self.thresh:
                continue
            obj_struct['xmin'], obj_struct['ymin'], obj_struct['xmax'], obj_struct['ymax'] = boxes[i, :4]
...
if __name__ == '__main__':
    ...
    print("output sub img path :{}".format(args.output_dir))
    pathExit(args.output_dir)

    assert os.path.exists(args.cfg_file)
    assert os.path.exists(args.model)
    assert os.path.exists(args.test_dir)

    model = mycaffe2(args.cfg_file, args.model,
                     thresh_=args.thresh, gpu_id=GPU_ID)
    generator = LabelGenerator(output_path=args.output_dir)

    detList = readTxt2Lines(args.test_dir)
    alllen = len(detList)
    print("{} imgs".format(alllen))

    for idx, imgPath in enumerate(detList):
        print('all length: {}, loading {},  name: {}'.format(alllen, idx, imgPath))
        try:
            assert os.path.exists(imgPath)
        except:
            print("{}, this img is not exist".format(imgPath))
            pass
        
        postfix = os.path.splitext(imgPath)[1]
        name = imgPath.strip().split('/')[-1].replace(postfix, '') # basename

        predict_list, im = model.detect(imgPath)
        ...
        if predict_list:
            ...
            generator.gen_labels(imgPath, im, predict_list)
