...
def parse_args():
    parser = argparse.ArgumentParser(description='Detectron api, Input test_imgs Dir, Output xml')
    parser.add_argument(
        '--m',
        dest='model',
...
class mycaffe2(object):
    def __init__(self, cfg_file, weights, gpu_id=0, thresh_=0.5):
        self.gpu_id = gpu_id
        self.thresh = thresh_
        self.classs = dummy_datasets.get_surveyPOSM_dataset()
...
if __name__ == '__main__':
    ...
    for idx, nname in enumerate(detList):
        ...
        outxml_path = os.path.join(output_dir, nname.strip().replace('.jpg', '.xml'))
	try:
